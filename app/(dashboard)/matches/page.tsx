    ...
    <div className="space-y-8 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-border">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-extrabold tracking-tight font-heading text-foreground">LỊCH SỬ TRẬN ĐẤU</h1>
            <p className="text-sm text-muted-foreground mt-0.5">Lịch sử đối đầu, kết quả tỉ số, số người tham gia và chia sẻ chi phí</p>
          </div>
        </div>
            ...
            return (
              <div 
                ...
                className={`player-card-glow relative p-6 rounded-2xl border bg-card/30 backdrop-blur-md transition-all hover:shadow-md flex flex-col justify-between ${
                  isWin ? "border-emerald-500/20 shadow-[0_4px_20px_-10px_rgba(16,185,129,0.1)]" :
                  isLoss ? "border-red-500/20 shadow-[0_4px_20px_-10px_rgba(239,68,68,0.05)]" :
                  "border-border/80"
                }`}
              >
                {/* Header info */}
                ...
                <div className="flex items-center justify-between gap-4 py-3 border-b border-border/30 pb-4">
                  ...
                  <div className="flex flex-col items-center w-[38%] text-center">
                    ...
                    <span className="font-extrabold text-sm text-foreground line-clamp-1">{match.opponent}</span>
                  </div>
                </div>
                ...
                <div className="mt-4 space-y-3">
                  ...
                  {match.scorers && (
                    <div className="flex items-start gap-2 text-xs text-foreground p-2 rounded-lg bg-primary/5 border border-primary/10">
                      ...
                      <div>
                        <span className="font-bold text-[10px] uppercase tracking-wider text-muted-foreground block mb-0.5">Ghi bàn:</span>
                        <span className="font-medium text-foreground">{match.scorers}</span>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="p-3.5 rounded-xl bg-card border border-border/70 mt-3 space-y-2">
                    <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground block border-b border-border/40 pb-1.5">Phân bổ chi phí sân</span>
                    
                    <div className="grid grid-cols-3 gap-2 text-center text-xs divide-x divide-border/40 pt-1">
                      <div>
                        <span className="text-[10px] text-muted-foreground block font-medium">Tiền sân</span>
                        <span className="font-bold text-foreground">{match.pitchFee.toLocaleString('vi-VN')} ₫</span>
                      </div>
                      <div>
                        <span className="text-[10px] text-muted-foreground block font-medium">Số người</span>
                        <span className="font-extrabold text-foreground flex items-center justify-center gap-0.5">
                          <Users className="w-3.5 h-3.5 text-primary" /> {match.playersCount}
                        </span>
                      </div>
                      <div>
                        <span className="text-[10px] text-muted-foreground block font-medium">Phí / Cầu thủ</span>
                        <span className="font-black text-primary">{feePerPerson.toLocaleString('vi-VN')} ₫</span>
                      </div>
                    </div>
                  </div>
                </div>
