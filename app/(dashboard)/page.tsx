...
import { EXPENSE_SOURCE, FUND_AMOUNT, FUND_STATUS, MEMBER_STATUS } from "@/lib/constants"
...
type ScheduleItem = Awaited<ReturnType<typeof db.schedule.findMany>>[number]
type MatchItem = Awaited<ReturnType<typeof db.match.findMany>>[number]
type PlayerStatItem = {
  memberId: number
  goals: number
  member: {
...
export default async function DashboardPage({
  ...
  const [
    ...
    [schedulesCount, recentMatches, upcomingSchedules, playerGoalStats, activeMembers],
  ] = await Promise.all([
    Promise.all([
      db.member.count({
    ...
    Promise.all([
      ...
      db.playerMatchStat.findMany({
        where: dateFilter
          ? { goals: { gt: 0 }, match: { date: dateFilter } }
          : { goals: { gt: 0 } },
        include: {
          member: {
            select: {
  ...
  ;(playerGoalStats as PlayerStatItem[]).forEach((stat) => {
  ...
  const formMatches = (recentMatches as MatchItem[]).slice(0, 5).reverse()
      ...
      <div className="grid gap-8 grid-cols-1 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="rounded-2xl border border-white/5 bg-card/20 backdrop-blur-md p-6 space-y-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.05)]">
            ...
            {upcomingSchedules.length === 0 ? (
              ...
              <div className="space-y-3">
                {(upcomingSchedules as ScheduleItem[]).map((schedule) => (
                  <div
                    key={schedule.id}
          ...
          <div className="rounded-2xl border border-white/5 bg-card/20 backdrop-blur-md p-6 space-y-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.05)]">
            ...
            {(recentMatches as MatchItem[]).length === 0 ? (
              <div className="text-center py-12 text-xs text-muted-foreground border border-dashed border-border/50 rounded-xl">
                Chưa có dữ liệu kết quả trận đấu được nhập.
              </div>
            ) : (
              <div className="space-y-3">
                {(recentMatches as MatchItem[]).map((match) => (
                  <div
                    key={match.id}
                    className="flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-xl border border-border/60 bg-background/50 hover:bg-muted/10 transition-all text-xs"
                  >
